...
版本列表请去这里获取：${Green_font_prefix}[ https://github.com/txthinking/brook/releases ]${Font_color_suffix}"
	...
	if [[ -z ${brook_new_ver} ]]; then
		...
		echo -e "${Info} 检测到 Brook 默认版本为 [ ${brook_new_ver} ]"
	else
...
Download_brook(){
	...
	cd ${file}
	if [[ ${bit} == "x86_64" ]]; then
		wget --no-check-certificate -N "https://github.com/jerry0591/bash_shell/blob/main/brook/brook"
	else
		wget --no-check-certificate -N "https://github.com/jerry0591/bash_shell/blob/main/brook/brook_linux_386"
